...
def load_actor(run: Path, device: torch.device) -> Actor:
    ...
    if "limbs" in state:
        ...
        policy = LimbPolicy(limbs=int(state["limbs"]), message=int(state["message_dim"]),
                            ...
                            oracle=int(state["actor.oracle"]), peek=bool(state.get("peek", False)),
                            identity=bool(state.get("identity", False))).to(device)
        state.setdefault("peek", torch.tensor(False))
        state.setdefault("identity", torch.tensor(False))
...
def distill(*, teacher: Path, output: Path, device: str, worlds: int, iterations: int, seed: int,
    ...
    if initial:
        ...
        student = LimbPolicy(limbs=limbs, message=message, hidden=hidden, fine_scales=fine_scales, peek=peek,
