...
| Robots | Tuned PID | Memoryless net (PPO) | Recurrent net (PPO) | **Distilled student, 384 units** | Its oracle teacher |
| --- | --- | --- | --- | --- | --- |
| Healthy | 97.6% | 77.9% | 87.8% | **98.4%** | 100% |
| Defective | 25.2% | 35.8% | 50.0% | **83.3%** | 96.2% |
| Defective, changing mid-episode | 22.2% | 33.9% | 46.2% | **83.3%** | 95.1% |
| ... and pushed by a neighbour (never trained on) | 12.6% | 26.1% | 32.1% | **51.8%** | 82.1% |

Median final error on defective, changing robots: PID 142 mrad, student 2.4 mrad. The student also gets there
...
4. Training the *teacher* against a stricter tolerance than the 30 mrad success criterion, wider and longer.
   ...
   86.5% and its student 74%; at 10 mrad (192 units, 600 iterations) 93.4% and 81%; at 5 mrad (256 units,
   800 iterations) 95.1% and 83.3% with a 384-unit student. The student tracks its teacher about 12 points
   below, so the teacher is the lever; a wider student (256 to 384 units) adds about two points.
...
scripts/project-run .venv/bin/python -m robot_ai.train.reach train --fine-error --oracle full --hidden 256 \
  --reward-tolerance 0.005 --iterations 800 --output artifacts/reach/oracle

# Deployable student by distillation (about an hour).
scripts/project-run .venv/bin/python -m robot_ai.train.reach distill --teacher artifacts/reach/oracle --hidden 384 \
